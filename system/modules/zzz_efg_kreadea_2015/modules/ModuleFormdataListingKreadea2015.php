...
/**
 * Class ModuleFormdataListingKreadea2015
 * Anpassung Erweiterung der Klasse Efg\ModuleFormdataListing für Kreadea
 ...
 * @package    Efg
 */
class ModuleFormdataListingKreadea2015 extends \ModuleFormdataListing
{
					...
					if (strlen(\Input::get('search')) && is_array(\Input::get('for')))
					{
                        ...
                        foreach (\Input::get('for') as $field => $for)
						{
							if (in_array($field, $arrSearchFields) && strlen($for))
							{
								...
								{
									/** Beginn Änderung (Block 1) Marko Cupic Mai 2015 **/
                                     // $arrConds[] = '(SELECT value FROM tl_formdata_details WHERE ff_name="' .$field. '" AND pid=f.id ) LIKE ?';
									 // $arrKeywords[] = '%' . urldecode($for) . '%';

                                    if($field == 'anform_plz' || $field == 'geform_plz'){

                                        if($arrVals['umkreis'] > 0){

                                            $degam_link = @mysql_connect($GLOBALS['open_geo_db']['host'], $GLOBALS['open_geo_db']['user'], $GLOBALS['open_geo_db']['password']);
                                            @mysql_select_db($GLOBALS['open_geo_db']['database'], $degam_link);
                                            ...
                                            $sql_rad = 'SELECT ' .
                                                ...
                                                'WHERE src.zc_id IN (SELECT zc_id FROM zip_coordinates WHERE zc_zip LIKE "'.urldecode($for).'%") ' .
                                                'AND dest.zc_id <> src.zc_id ' .
                                                'HAVING distance < ' . $arrVals['umkreis'] . ' ' .
                                                'ORDER BY distance';

                                            //echo $sql_rad . "<br><br>";

                                            $degam_result = mysql_query($sql_rad);
                                            //echo mysql_error();

                                            $degam_zips = urldecode($for);

                                            while($degam_rec = mysql_fetch_object($degam_result)){
                                                if( $degam_zips){
                                                    $degam_zips .= ',';
                                                }
                                                $degam_zips .= $degam_rec->zc_zip;
                                            }

                                            mysql_close($degam_link);

                                            //echo $degam_zips;

                                            $arrConds[] = '(SELECT value FROM tl_formdata_details WHERE ff_name="' .$field. '" AND pid=f.id ) IN ('.$degam_zips.')';
                                            $arrKeywords[] = '';
                                        ...
                                        if(($field == 'anform_richtung' && urldecode($for) != '0') || $field != 'anform_richtung'){
                                            if($for != "0"){
                                                $arrConds[] = '(SELECT value FROM tl_formdata_details WHERE ff_name="' .$field. '" AND pid=f.id ) LIKE "%' . urldecode($for) . '%"';
                                                $arrKeywords[] = '';
                                            }
                                        }
                                    }
                        ...
                        /** Ende Änderung (Block 1) Marko Cupic Mai 2015 **/
